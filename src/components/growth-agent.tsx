...
type ChatMessage = { id: string; role: "user" | "assistant"; content: string; steps?: AgentStep[]; provider?: Provider; model?: string; createdAt: number; status?: "pending" | "failed" | "unsaved" };
...
const toolLabel: Record<string, string> = { get_analytics_summary: "Analytics", get_competitors: "Competitors", get_trend_radar: "Trend radar" };
...
function normalizeMessage(value: unknown): ChatMessage | null {
  ...
  if ((m.role !== "user" && m.role !== "assistant") || typeof m.id !== "string" || typeof m.content !== "string") return null;
  return { id: m.id, role: m.role, content: m.content, steps: Array.isArray(m.steps) ? m.steps : [], provider: m.provider, model: m.model, createdAt: typeof m.createdAt === "number" ? m.createdAt : Date.now(), status: m.status === "pending" || m.status === "failed" || m.status === "unsaved" ? m.status : undefined };
}
function normalizeConversation(value: unknown): Conversation | null {
  if (!value || typeof value !== "object") return null;
  const c = value as Partial<Conversation>;
  if (typeof c.id !== "string" || typeof c.title !== "string" || !Array.isArray(c.messages)) return null;
  return { id: c.id, title: c.title, messages: c.messages.map(normalizeMessage).filter((m): m is ChatMessage => Boolean(m)), createdAt: typeof c.createdAt === "number" ? c.createdAt : Date.now(), updatedAt: typeof c.updatedAt === "number" ? c.updatedAt : Date.now(), archived: Boolean(c.archived) };
}
function loadConversations(): Conversation[] { if (typeof window === "undefined") return []; try { const parsed = JSON.parse(localStorage.getItem(STORAGE_KEY) || "[]") as unknown; return Array.isArray(parsed) ? parsed.map(normalizeConversation).filter((v): v is Conversation => Boolean(v)) : []; } catch { return []; } }
function saveConversations(value: Conversation[]) { if (typeof window !== "undefined") { try { localStorage.setItem(STORAGE_KEY, JSON.stringify([...value].sort((a, b) => b.updatedAt - a.updatedAt).slice(0, MAX_CONVERSATIONS))); } catch {} } }
...
export function GrowthAgent() {
  ...
  const refreshProviders = async () => { try { const res = await fetch("/api/agent", { cache: "no-store" }); const data = await res.json(); if (data?.providers) setProviders(data.providers as Providers); } catch {} };

  useEffect(() => { const stored = loadConversations(); const initial = stored.length ? stored : [makeConversation()]; setConversations(initial); setActiveId(initial[0].id); const saved = localStorage.getItem(MODEL_KEY); if (saved?.includes(":")) { const [p, ...parts] = saved.split(":"); if (["openai", "anthropic", "zai", "gemini"].includes(p)) { setProvider(p as Provider); setModel(parts.join(":")); } } void refreshProviders(); }, []);
  ...
  useEffect(() => { const models = providers[provider].models; if (!models.length) return; if (!models.some((m) => m.id === model)) { const first = models.find((m) => !unavailable[`${provider}:${m.id}`]) ?? models[0]; setModel(first.id); } }, [provider, providers, model, unavailable]);

  const active = useMemo(() => conversations.find((c) => c.id === activeId) ?? null, [activeId, conversations]);
  const visible = useMemo(() => { const needle = search.trim().toLowerCase(); return conversations.filter((c) => Boolean(c.archived) === showArchived && (!needle || c.title.toLowerCase().includes(needle) || c.messages.some((m) => String(m.content ?? "").toLowerCase().includes(needle)))); }, [conversations, search, showArchived]);
  ...
  function deleteConversation(id: string) { const remaining = conversations.filter((c) => c.id !== id); if (id !== activeId) { setConversations(remaining); return; } const sameTab = remaining.find((c) => Boolean(c.archived) === showArchived); const next = sameTab ?? remaining[0] ?? makeConversation(); setConversations(remaining.length ? remaining : [next]); setActiveId(next.id); if (next.archived !== showArchived) setShowArchived(next.archived); }
  ...
  async function sendMessage(text: string) {
    ...
    const pendingId = makeId(); const userMessage: ChatMessage = { id: pendingId, role: "user", content: trimmed, createdAt: Date.now(), status: "pending" }; const history = [...active.messages, userMessage]; updateActiveMessages(history); setInput(""); setBusy(true); setError(null);
    try {
      ...
      if (!res.ok) { setError(data?.error ?? "Growth Operator could not answer right now."); setChatsAfterFailure(pendingId); return; }
      const assistant: ChatMessage = { id: makeId(), role: "assistant", content: typeof data?.answer === "string" ? data.answer : "I didn't get an answer back.", steps: Array.isArray(data?.steps) ? data.steps : [], provider: data?.provider, model: data?.model, createdAt: Date.now() };
      try { updateActiveMessages([...history.map((m) => m.id === pendingId ? { ...m, status: undefined } : m), assistant]); } catch { updateActiveMessages([...history, { ...assistant, status: "unsaved" }]); setError("Answer generated but could not be displayed correctly."); }
    } catch (err) { setChatsAfterFailure(pendingId); setError(err instanceof Error ? err.message : "Something went wrong."); }
    finally { setBusy(false); requestAnimationFrame(() => inputRef.current?.focus()); }
  }

  function setChatsAfterFailure(messageId: string) { if (!active) return; updateConversation(active.id, { messages: active.messages.map((m) => m.id === messageId ? { ...m, status: "failed" } : m) }); }
  function submit(e: FormEvent<HTMLFormElement>) { e.preventDefault(); void sendMessage(input); }

  return <section className="flex h-[calc(100vh-9rem)] min-h-[620px] overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
