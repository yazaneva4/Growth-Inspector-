  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/settings/accounts", {
        ...
        body: JSON.stringify({ platform, externalId, displayName }),
      });
    ...
    <div>
          ...
          {accounts.map((a) => (
            ...
            >
              <div>
                ...
                <span className="ml-2 text-slate-500" dir="auto">{a.display_name}</span>
                <span className="ml-2 text-xs text-slate-400">({a.external_id})</span>
              </div>
              <button
      ...
      <form onSubmit={submit} className="grid gap-2.5 sm:grid-cols-2">
        ...
        <input
          ...
          placeholder="Display name (e.g. Main support line)"
          required
          className="rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm outline-none focus:border-emerald-500"
        />
        <input
          ...
          placeholder={PLATFORM_ID_HINT[platform] ?? "Account id"}
          required
          className="sm:col-span-2 rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm outline-none focus:border-emerald-500"
        />
        <button
          type="submit"
          disabled={busy}
