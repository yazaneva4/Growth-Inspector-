...
type Message = { id: string; role: "user" | "assistant"; content: string; provider?: RealProvider; model?: string; createdAt: number };
...
export function GrowthAiChat() {
  ...
  async function refreshChats(selectFirst = false) {
    try {
      const res = await fetch("/api/agent/conversations", { cache: "no-store" }); const data = await res.json().catch(() => null); if (!res.ok) throw new Error(data?.error || "Could not load saved conversations.");
      const next = (Array.isArray(data?.conversations) ? data.conversations : []).map((item: ApiConversation) => fromApi(item));
      if (!next.length) { const create = await fetch("/api/agent/conversations", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "create" }) }); const created = await create.json().catch(() => null); if (!create.ok || !created?.conversation) throw new Error(created?.error || "Could not create a conversation."); const chat = fromApi({ ...created.conversation, ai_operator_messages: [] }); setChats([chat]); activeIdRef.current = chat.id; setActiveId(chat.id); return; }
  ...
  async function send(text: string) {
    ...
    const history = historyForModel([...current.messages, { id: "pending", role: "user", content: value, createdAt: Date.now() }]);
