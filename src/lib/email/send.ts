...
function configuredFrom(user?: string) {
  ...
  // The example placeholder must never become the real sender address.
...
export async function sendEmail(mail: OutboundEmail): Promise<boolean> {
  ...
  if (kind === "none") {
    throw new Error(
      "Growth Inspector application email is not configured. Set SMTP_HOST/SMTP_USER/SMTP_PASSWORD or GMAIL_USER/GMAIL_APP_PASSWORD in the deployment environment.",
    );
  }
