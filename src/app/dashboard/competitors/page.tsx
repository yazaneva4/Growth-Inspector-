...
const badge: Record<string, string> = {
  ...
  x: "bg-slate-500/20 text-slate-200",
    ...
    <div className="max-w-3xl">
      ...
      <div className="mt-5 space-y-3">
        {rows.length === 0 ? (
          ...
          rows.map((c) => (
            <div key={c.id} className="rounded-2xl border border-slate-800 bg-slate-900/60 p-5">
              <div className="flex items-center gap-2">
                <span className={`rounded-md px-2 py-0.5 text-[10px] font-bold uppercase ${badge[c.platform] ?? "bg-slate-500/20 text-slate-300"}`}>
                  {c.platform}
                </span>
                <span className="font-medium" dir="auto">{c.handle}</span>
              </div>
              {c.notes && (
                <p className="mt-2 text-sm text-slate-400" dir="auto">{c.notes}</p>
              )}
            </div>
          ))
        )}
